...
from generator.llm.gemini import Gemini
...
class Cli:
    ...
    def generate_syzlange(self, cuda_api_text):
        ...
        self.controller.analyze(
            cuda_api_text,
            self.chat_type,
            self._join_syzlang_json,
            self._refer_syzlang_context,
        )
    ...
    def _init_logger(
        self, cus_format="%(asctime)s %(message)s", debug=False, propagate=False
    ):
